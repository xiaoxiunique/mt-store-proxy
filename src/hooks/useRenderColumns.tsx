...
export function useGenColumns(fields: string[], metadata: any = {}) {
  ...
  const columns: TableColumn[] = fields.map(field => {
    ...
    if (metadata.default && !metadata["formatter"]) {
      // @ts-ignore
      column.formatter = row => row[column["prop"]] || metadata.default;
    }
  ...
  return [...defaultColumns, ...columns, createdAtColumn, actionColumn];
}
